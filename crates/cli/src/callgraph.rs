...
pub enum ResolutionHint {
    /// A definition named `callee_name` exists in this call site's own
    /// file — the strongest signal available without real scope
    /// resolution, and right far more often than not in real code.
    LikelySameFile,
    ...
    /// file — genuinely ambiguous without real resolution; shown as-is,
    ...
    /// elsewhere, dynamic dispatch, or simply undexed code.
    NoDefinitionIndexed,
}
...
pub fn find_callers(conn: &Connection, callee_name: &str, limit: usize) -> Result<Vec<CallerHit>> {
    // One query, up front, for every file that defines something named
    // `callee_name` — cheap (an indexed name lookup), and reused for
    // every hit below instead of re-querying per row.
    let mut def_stmt = conn.prepare("SELECT DISTINCT f.path FROM symbols s JOIN files f ON f.id = s.file_id WHERE s.name = ?1")?;
    let def_files: HashSet<String> = def_stmt.query_map([callee_name], |r| r.get::<_, String>(0))?.filter_map(|r| r.ok()).collect();
    ...
    Ok(rows
        ...
        .map(|(path, kind, name, line)| {
            let resolution = if def_files.is_empty() {
                ResolutionHint::NoDefinitionIndexed
            } else if def_files.contains(&path) {
                ResolutionHint::LikelySameFile
            } else if def_files.len() == 1 {
                ResolutionHint::UniqueElsewhere
            } else {
                ResolutionHint::Ambiguous
            };
...
mod tests {
    ...
    fn likely_same_file_when_a_definition_shares_the_call_sites_file() {
        ...
        assert_eq!(a.resolution, ResolutionHint::LikelySameFile);
        assert_eq!(b.resolution, ResolutionHint::UniqueElsewhere);
    }
    ...
    fn ambiguous_when_multiple_definitions_exist_and_none_match_the_call_site() {
        ...
        assert_eq!(hits[0].resolution, ResolutionHint::Ambiguous);
    }
