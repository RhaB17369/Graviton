...
`use`/`import` statement** naming that exact definition (54 languages —
...
**54 languages** — every parsed language except WGSL (extracted, but
deliberately never resolved — see below). Run `grv languages` any time
for the live, authoritative list (its own "import resolution" line, not
just this doc): Rust (crate/module tree, discovered from every
...
R/Racket/CMake/Erlang/Zig/PHP/LaTeX/Dart/Scheme/PowerShell/Assembly/
Crystal/Prolog/GraphQL (quoted/relative-literal path resolution —
`#include "x"` resolved against the including file's own directory, real
search-path semantics, while `#include <x>` is always a system header,
correctly never even recorded; GraphQL's is the `graphql-import` community
comment convention, not core spec syntax), Java/Kotlin/Groovy/Scala/C#
(hierarchical module-name resolution against conventional Maven/Gradle-
style source roots, with `import a.b.*`-style wildcards resolving to
every file in the target package directory — same multi-file honesty as
Go), Elm/Haskell/D/Julia (the same hierarchical resolution, but a
wildcard/unqualified/`exposing (..)`/`hiding` import resolves to that one
module's own file, never a directory listing — these are
one-module-per-file languages, not package languages; an earlier version
of this resolver got Elm's case wrong exactly this way before the
distinction was made explicit), Lua/Nim (dotted-or-slash-to-path
resolution, Nim also checking the importing file's own directory first,
its real lookup order), Ada/OCaml/Perl/Fortran/VHDL/Elixir, each via its
own genuinely different naming convention (GNAT's dash-joined-lowercase
flat naming; OCaml's lowercase-outermost-segment; Perl's `::`-to-`/` CPAN
convention; a flat filename guess for Fortran and VHDL, neither of which
has a real, enforced naming convention — VHDL's is scoped to the `work`
library only, its own unambiguous "compiled from this repo" marker; Mix's
CamelCase-to-snake_case under `lib/` for Elixir), Swift (Swift Package
Manager's own target-to-directory convention: `import CoreModule`
...
framework has no matching directory and stays honestly unresolved),
...
extracted), and Svelte/Vue (the real imports inside a `<script>` block
are ordinary JS/TS; a real second parse pass extracts them, then resolves
them exactly like a plain `.js`/`.ts` file would). An import that can't
be resolved (an external crate/package/system header, the stdlib, a
`tsconfig.json` path alias, a non-standard Rust `#[path]` layout, PSR-4
autoloading for PHP `use`, a multi-segment OCaml `open` naming a
sub-module rather than a compilation unit, a Swift `import` of an
external framework, WGSL's naga_oil `#import` — extracted as real signal,
but never resolved, since a naga_oil module name has no fixed
file-naming convention at all to guess against) is left unresolved —
never a wrong guess. This is what powers `ResolutionHint::ImportResolved`
above; see ARCHITECTURE.md's "Import resolution" section for exactly what
each language's resolver can and can't do.
...
- Call-graph *type/full scope* resolution — still name-based by design; a real import resolver now exists for every parsed language except one (see "Import resolution" above, and `ResolutionHint::ImportResolved`), but true type resolution (knowing exactly which overload/trait impl a call targets) is a different order of engineering effort, on par with what a language server spends its whole existence on. Only WGSL is extracted but never resolved — naga_oil's `#import a::b` names an arbitrarily-registered shader module, not a file-per-module convention, so there's nothing reliable to resolve against; guessing off a same-named file anywhere in the repo would be exactly the fuzzy match this project's resolvers are built to never do.
